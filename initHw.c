...
void initHw()
{
   ...
   //GPIOIntTypeSet(GPIO_PORTC_DATA_BITS_R, GPIO_PIN_6, GPIO_LOW_LEVEL);
   //GPIOIntEnable (GPIO_PORTC_DATA_BITS_R, GPIO_PIN_6);
   //IntEnable(INT_GPIOC);

   GPIO_PORTC_IS_R=  0x00;  // level trigger
   GPIO_PORTC_IEV_R  =0xC0;  // low level detection from high to low
   GPIO_PORTC_IM_R  =0xC0;
   ...
   // Configure port D for ext push button
    ...
    GPIO_PORTD_IS_R=  0x00;  // level trigger
    GPIO_PORTD_IEV_R  =0x40;  // low level detection from high to low
